...
("filterMaxX",  po::value<int>(), "filter according maximal X coordinate of the barycenter connected component. The CC is given only of its barycenter is less or eq than [arg]  " )
  ...
  if(vm.count("volImageName") &&(vm.count("sliceX")||vm.count("sliceY")||vm.count("sliceZ"))){
    ...
    if(vm.count("sliceX")){
      ...
      SliceImageAdapter sliceImageX(img, domain2DX, aSliceFunctorX, identityFunctor);
      viewer << sliceImageX;
    ...
    if(vm.count("sliceZ")){
      ...
      SliceImageAdapter sliceImageZ(img, domain2DZ, aSliceFunctorZ, identityFunctor);
      viewer << sliceImageZ;
    ...
    if(vm.count("sliceY")){
      ...
      SliceImageAdapter sliceImageY(img, domain2DY, aSliceFunctorY, identityFunctor);
      viewer << sliceImageY;
